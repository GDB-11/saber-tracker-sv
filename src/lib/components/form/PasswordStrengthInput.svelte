<!-- src\lib\components\form\PasswordStrengthInput.svelte -->
<script lang="ts">
...
<div class="space-y-1">
    ...
    <div class="relative">
        ...
        >
            {#if showPassword}
                <EyeOffSvg class="h-5 w-5 text-gray-500 dark:text-gray-400" />
            {:else}
                <EyeOnSvg class="h-5 w-5 text-gray-500 dark:text-gray-400" />
            {/if}
        </button>

        <!-- State icons (optional, similar to Input.svelte) -->
        {#if currentVariant === 'success'}
            <div class="absolute inset-y-0 right-10 flex items-center pr-3 pointer-events-none">
